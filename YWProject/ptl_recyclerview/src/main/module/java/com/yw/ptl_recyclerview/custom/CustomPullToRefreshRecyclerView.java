...
import com.yw.ptl_recyclerview.DefaultHeaderAndFooterCreator.DefaultRefreshHeaderCreator;
import com.yw.ptl_recyclerview.HeaderAndFooter.HeaderAndFooterRecyclerView;
import com.yw.ptl_recyclerview.PullToRefresh.OnRefreshListener;
import com.yw.ptl_recyclerview.PullToRefresh.RefreshHeaderCreator;

/**
 * Created by Administrator on 2016/9/19.
 * HeaderView : topView(max, 1)
 *
 * DefaultRefreshHeaderCreator : 刷新头部View
 * RefreshHeaderCreator : 设置自定义的刷新头部
 * 单一责任原则：凡是继承RefreshHeaderCreator而自定义的头部
 *  责任是：创建头部view与监听RecyclerView的触摸各种状态（下拉、松手等）与各种状态下头部view的动态效果
 *
 * 真实Adapter 与 虚拟Adapter的分工与作用各是什么
 *
 * 测量 - 布局 - 绘制
 * 1.测量头部View的下拉的高度
 * 2.监听触摸Touch事件，判断每个临界点的事件处理
 * 3.临界点的状态与动画交互
 */
public class CustomPullToRefreshRecyclerView extends HeaderAndFooterRecyclerView {
    ...
    public CustomPullToRefreshRecyclerView(Context context, AttributeSet attrs, int defStyle) {
...
//            初始化默认的刷新头部
            ...
            mRefreshView.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    Toast.makeText(context, "小程序被点击了", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
...
//                若向上滑动(此时刷新头部已隐藏)，不处理  - 可以加个动画回弹（-120 < distance < 0） 最好判断headerView是否可见
                if (distance < 0) {
                    Log.e("YW-e", "move 拦截 :" + distance);
                    break;
                }
                Log.e("YW-e", "move distance :" + distance);
                mPulling = true;
//                若刷新中，距离需加上头部的高度
